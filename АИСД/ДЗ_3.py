...
class Graph:
    ...
    def __init__(self, row, col, nodes=None): # установка матрица смежности
        self.adj_mat = [[0] * col for _ in range(row)] # Связывает node1 с node2
        self.nodes = nodes
        for i in range(len(self.nodes)):
            self.nodes[i].index = i

    # Связывает node1 с node2
    # обновление для поддержки взвешанных рёбер
    def connect_dir(self, node1, node2, weight=1):
        node1, node2 = self.get_index_from_node(node1), self.get_index_from_node(node2)
        self.adj_mat[node1][node2] = weight

    # Вес между двумя нодами
    def connect(self, node1, node2, weight=1):
        self.connect_dir(node1, node2, weight)
        self.connect_dir(node2, node1, weight)

    # Получает ряд узла, отметить ненулевые объекты с их узлами в массиве self.nodes
    # Выбирает любые ненулевые элементы, оставляя массив узлов которые являются connections_to (для ориентированного графа)
    # Возвращает значение: массив (узел, вес)
    ...
    # Выбирает только ненулевые элементы, при этом connections_to ИЛИ connections_from используются для неориентированного граф
    # Возвращает значение: массив (узел, вес)
